...
void main() {
  ...
  group('OfflineTransport', () {
    ...
    test('serves form POSTs keyed by url and payload, throws on a miss',
        () async {
  ...
  group('HttpTransport', () {
    test('throws a clear, actionable message for a Cloudflare challenge',
      ...
      try {
        ...
        expect(e.sessionExpired, isFalse,
    ...
    test('POSTs JSON bodies with application/json and returns decoded JSON',
        () async {
      final mock = MockClient((request) async {
        expect(request.method, 'POST');
      ...
      expect(value, {'success': true, 'data': [{'id': 1}]});
    });
  });
}
