...
void main() {
  test(
    'atlas-plugins/index.json is current: every file in atlas-plugins/ '
    'matches its committed checksum and version',
    () async {
      ...
      for (final entry in entries.whereType<Directory>()) {
        ...
        await for (final file in entry.list()) {
          ...
          expect(actual, expected,
              reason: 'working tree "$id/$name" no longer matches the index; '
                  're-run the tool');
        }
      }
    },
