  ...
  group('ImagePipeline', () {
    test('downloads bytes and stores them content-addressed', () async {
      ...
      final transport = _FakeTransport(
        bytes: {'https://example.com/a.png': bytes},
      );
    ...
    test('infers extension from the URL', () async {
      ...
      final path = await pipeline.download(
        Uri.parse('https://example.com/c.jpg'),
      );
    ...
    test('dedupes identical content to a single file', () async {
      ...
      final first = await pipeline.download(
        Uri.parse('https://example.com/a.png'),
      );
      final second = await pipeline.download(
    ...
    test(
      'skips the network fetch when the content hash already exists',
      () async {
        final bytes = [9, 8, 7];
        final transport = _FakeTransport(
          bytes: {'https://example.com/a.png': bytes},
    ...
    test('returns null on empty response', () async {
      ...
      final path = await pipeline.download(
        Uri.parse('https://example.com/e.png'),
      );
